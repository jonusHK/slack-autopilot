...
- 상태: 초안 (2026-08-14) — 3단계(검출·분류)까지 구현된 시점의 기록.
...
| 무엇 | 어디 | 비고 |
|---|---|---|
| 깨우는 것 | 클라우드 트리거 3개(:00·:20·:40) | **최소 간격 1시간**이라 오프셋으로 20분을 만든다. KST 07~02시 |
...
| git 자격이 있다 | **없다** — 프록시가 `gh` 경로만 갈아 끼운다 | `git clone https://github.com/…` 이 프라이빗 레포에서 죽음 |
...
- **프롬프트가 레포에 있는 이유**: 트리거는 오프셋 때문에 여러 개가 되고, 프롬프트를 트리거에
...
| `could not read Username for 'https://github.com'` | **VM 에 GitHub 자격이 없다.** 프록시 경로를 타는 것은 `gh` 뿐 — 맨 `git clone` 은 프라이빗 레포에서 죽는다 | `gh repo clone <owner>/<repo>` 를 쓴다 |
...
| 2. 엔진 스크립트 + 정책 파일 | ✅ 2026-08-14 (`bin/*`, sai `AUTOMATION.md`) |
| 3. 검출·분류 루틴 | ✅ **작동 확인**(2026-08-14 실채널 관통). 트리거 3개 :00·:20·:40, KST 07~02시 — 분류 정확도 관찰 중 |
| 4. 구현·PR + 자기 수정 | 🔶 **명세·도구 준비 완료**(`prompts/implement.md`·`lock.py`) — 트리거 미등록 |
| 5. 병합 루틴(🚀) | 🔶 **명세 준비 완료**(`prompts/merge.md`·`detect --mode merge`) — 트리거 미등록 |
| 6. 동시 3개 확장 | ☐ 미구현 |

4·5단계는 **도구와 명세가 서 있고 트리거만 안 걸린 상태**다. 켜기 전에 확인할 것:
...
## 11. 실제로 돈 첫 사이클 (2026-08-14, #sai)

설계가 종이에서만 돌지 않았다는 기록. 상태 전이가 문서대로 갔다.

1. 사람: "QA - 260814" 메시지(불릿 2개) + ▶️
2. 봇: 💬 클레임 → 분류 → 스레드 답글 + ❓ — **자동 0건 / 결정 필요 2건**
   - 불릿 1은 코드를 읽어 "재공유가 막혀 있다"가 사실이 아님을 확인하고 판단 지점 셋으로 갈랐다.
   - 불릿 2는 물음표로 끝나 무조건 B.
3. 사람: 결정을 **답글로** 적고 그 답글에 ▶️(스크린샷 첨부)
4. 봇: 그 답글을 새 지시로 검출(D-006 재투입 루프가 실제로 작동) → 💬 → 처리
5. 결과를 스레드에 남기고 ✅

**배운 것**: 스크린샷이 결정적이었다. 봇 토큰에 `files:read` 가 없어 이번에는 사람이 대신
읽었다 — QA 에 이미지가 흔하므로 4단계 전에 스코프를 늘릴지 정해야 한다(미결).
