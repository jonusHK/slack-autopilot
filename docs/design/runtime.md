...
| 무엇 | 어디 | 비고 |
|---|---|---|
| 깨우는 것 | 클라우드 트리거(cron) | **최소 간격 1시간** — 더 짧게는 오프셋 트리거 여러 개 |
...
bin/
  ...
  detect.py      ▶️ 있고 💬 없는 노드 검출 → JSON
  mark.py        이모지 전이 + 스레드 답글
config.json      채널↔레포 매핑(값이 아니라 **환경변수 이름**)
prompts/
  triage.md      3단계 루틴의 명세(트리거 프롬프트가 이 파일을 읽는다)
tests/
  test_detect.py 슬랙 없이 도는 규칙 테스트
scripts/
...
- **프롬프트가 레포에 있는 이유**: 트리거는 오프셋 때문에 여러 개가 되고, 프롬프트를 트리거에
  넣으면 N 벌이 갈라진다. 트리거는 부트스트랩만 담고 명세는 `prompts/triage.md` 한 곳에 둔다.
...
- `reactions.add` 가 이미 붙어 있으면 `already_reacted` 를 준다. 이것을 **test-and-set** 으로
  ...
  그 노드를 건너뛴다.
- **이건 보조 장치다.** 구현 단계(4단계)의 진짜 락은 원격 브랜치 push 다(원자적).
  분류만 하는 지금은 이 정도로 충분하다.
...
1. **범용 휴리스틱**(엔진): 무엇을 할지가 정해져 있고 **어떻게만** 남았으면 A,
   **무엇에 재량**이 있으면 B. 물음표로 끝나거나 "검토"·"~어때"가 들어가면 **무조건 B**.
...
| 3. 검출·분류 루틴 | ✅ 등록(트리거 :05, KST 08~24시) — **분류 정확도 관찰 중** |
| 4. 구현·PR + 자기 수정 | ☐ 미구현 |
| 5. 병합 루틴(🚀) | ☐ 미구현 |
| 6. 동시 3개 확장 | ☐ 미구현 |

4단계 전까지 ▶️ 를 달면 **분류 답글까지만** 온다. 구현은 사람이 한다.
