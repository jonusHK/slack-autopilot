...
- 상태: 초안 (2026-09-18 갱신 — §3.5 ⑦ 허용 규칙) — **4단계 관통 확인**(슬랙 ▶️ → 구현 → CI 그린 → PR), 5단계 트리거 등록. 절차와 환경만 둔다(결정·설계는
  `../decisions.md`·`../design/`).
...
| GitHub | 레포를 붙이면(§3.5 ⑥) 클론·push·PR 이 **토큰 없이** 된다 — 프록시가 인증한다. **`GH_TOKEN` 은 넣지 않는다**(헤더가 읽히지도 않는다). CI 상태는 세션의 GitHub 도구로 읽는다 |
| 클라우드 루틴 | 구현 매시 :00 · 병합 매시 :30. KST 07~02시 (오프셋 3개로 20분화했다가 되돌림 — D-013) |
...
(`cron interval too short`). 더 짧은 주기가 **정말** 필요하면 오프셋 루틴 여러 개로 만들 수는
있다(`0 …` / `20 …` / `40 …` → 실질 20분). 겹쳐 돌아도 안전하다. 다만 **유휴 실행이 곧
비용**이라 그만큼 값을 치른다 — 실제로 20분으로 깔았다가 되돌렸다(D-013). 상태가 이모지에
...
**⑦ `allowed_tools` 에 엔진 명령의 허용 규칙을 더한다(D-014).** 루틴 세션은 auto mode
분류기 아래서 돌고, 분류기는 세션에 붙은 레포(대상 레포)만 신뢰한다. 엔진 클론·`bin/*.py`
실행은 "외부 코드"로 **실행마다 다르게** 거부됐다(2026-09-18). 허용 규칙은 분류기보다 먼저
결정적으로 판정된다. 정본은 [`prompts/allowed_tools.json`](../../prompts/allowed_tools.json) —
기본 도구 20개 **뒤에** `Bash(…)` 규칙을 더한 목록 그대로를 `session_context.allowed_tools` 에
넣는다(`<OWNER>` 만 실제 소유자로). ⑤ 와 어긋나지 않는다: 줄이는 게 아니라 더하는 것이다.

규칙은 명령과 **글자 단위로** 맞아야 한다. 트리거 프롬프트는 `prompts/bootstrap.md` 의
명령을 그대로 옮기고(치환 자리만 채운다), 규칙과 프롬프트를 바꿀 때는 **한 트리거에 먼저
반영해 수동 실행으로 확인한 뒤** 나머지에 같은 글자로 전파한다(CLAUDE.md 의 순서). 확인은
로그의 `permission_denied` 유무와 클론·검출 소요 시간으로 한다 — 실측(2026-09-18): 규칙에
맞는 `git clone …` 0.9초·`bin/detect.py …` 2.3초, 분류기로 가면 심사만 30~50초. 소요 시간이
규칙 적용의 증거이고, 며칠치 로그에 거부가 0건이어야 한다.

**프롬프트 본문에 "허용 규칙"·"분류기"를 쓰지 않는다** — 그 문장이 `[Auto-Mode Bypass]` 사유가
됐다. `rm -rf` 도 클론 앞에 두지 않는다 — 규칙이 있어도 심사 대상이다.
