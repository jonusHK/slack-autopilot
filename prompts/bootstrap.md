...
**세 트리거(:00·:20·:40)의 프롬프트는 아래와 글자까지 같아야 한다** — 단 3)의 엔트리 파일만
단계에 따라 다르다(분류 전용 `triage.md` / 구현까지 `implement.md`). 그 한 단어 외에는 동일하다. 트리거가 여럿이면
...
3) 노드가 있을 때만 **엔트리 명세**(분류 전용이면 prompts/triage.md, 구현까지면 prompts/implement.md)를 읽고 그 파일의 절차를 그대로 수행한다. 그 파일이 명세이고 이 프롬프트보다 우선한다.
